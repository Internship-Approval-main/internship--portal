...
app = FastAPI()
from mock_faculty import router as faculty_router
...
class InternshipDetails(BaseModel):
    ...
    manager_email: str    
    stipend_type: Literal["paid", "unpaid"]
    offer_letter_url: Optional[str] = None
    status: Optional[str] = "Pending Approval"                 # Manager Email OR Supervisor Email
...
async def save_student_internships(submission: StudentInternshipSubmission):
    ...
    exempt_institutions = ["IIT", "NIT", "IISC", "IIIT"]
    ...
    for placement in doc["placements"]:
        ...
        # =========================================================
        # ADDED: STATUS & APPROVAL LOGIC
        # =========================================================
        company_upper = placement.get("company", "").strip().upper()
        is_exempt = any(inst in company_upper for inst in exempt_institutions)
        
        # Safely fetch stipend type (defaults to "" if not provided)
        stipend_type = placement.get("stipend_type", "").strip().lower()

        if is_exempt:
            placement["status"] = "Approved"
        elif placement["campus_type"] == "off campus":
            placement["status"] = "Pending Approval"
        elif placement["campus_type"] == "on campus" and stipend_type == "unpaid":
            placement["status"] = "Pending Approval"
        else:
            placement["status"] = "Approved"
